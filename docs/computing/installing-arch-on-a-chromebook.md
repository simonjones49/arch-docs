...
Having already tried Manjaro and Endeavouros I have moved my main machine to [Arch Linux](https://archlinux.org){:target="_blank"}.
...
I did however only have a Dummy Output for a sound card, a quick check of the [Arch Wiki](https://wiki.archlinux.org){:target="_blank"} showed I needed to install two packages, so I did.
```
...
In actual fact the Arch install was easier than Manjaro and Endeavouros so never let it be said [Arch Linux](https://archlinux.org/){:target="_blank"} is hard.
